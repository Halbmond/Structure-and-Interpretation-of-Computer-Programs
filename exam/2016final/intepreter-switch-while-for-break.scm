...
(define (eval-switch value table env)
  ;assert table != '()
  ;(displayln value) (displayln table)
  (if (or (tagged-list? (car table) 'default) (equal? value (eval (caar table) env)))
    (last (map (lambda (exp) (eval exp env)) (cdar table)))
    (eval-switch value (cdr table) env)
  )
)
...
(define (eval-while exp env)
  (if (eval (car exp) env)
    (begin
      (for-each (lambda (exp) (eval exp env)) (cdr exp))
      (eval-while exp env)
    )
    (void)
  )
...
(define (eval-for exp env)
  (if (eval (car exp) env)
    (begin
      (if (eq? (eval-sequence (cddr exp) env) 'BREAK)
        (void)
        (begin
          (eval (cadr exp) env)
          (eval-for exp env)))
...
(define (eval exp env)
  (cond ((self-evaluating? exp ) exp)
        ...
        ((switch? exp) (eval-switch (eval (cadr exp) env) (cddr exp) env))
        ((while? exp) (eval-while (cdr exp) env))
        ((for? exp) (eval (cadr exp) env) (eval-for (cddr exp) env))
        ((break? exp) 'BREAK)
        ((lambda? exp)
...
(define (eval-sequence exps env)
  (define first-res (eval (first-exp exps) env))
  (if (or (last-exp? exps) (eq? first-res 'BREAK))
    first-res
